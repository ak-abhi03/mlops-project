...
numeric_cols = dataset.select_dtypes(include=['int64','float64']).columns.to_list()
...
dataset[numeric_cols] = dataset[numeric_cols].apply(lambda x: x.clip(*x.quantile([0.05, 0.95])))
...
def mlflow_logging(model, x, y, name):
    with mlflow.start_run(run_name=name+"".join(str(date.today()))) as run:
